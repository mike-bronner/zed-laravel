...
| Magic members | `$user->posts`, `$user->email` | every property-form usage (PHP + Blade) of a relationship / accessor / column |

**Eloquent magic is first-class.** Find references on a property-form usage site — `$user->posts`, `$model->full_name`, `$user->email` — and every indexed usage of that member surfaces, in PHP and Blade alike. The usage is resolved through the semantic index to its declaring class (inheritance- and trait-aware), so the results are keyed by what the member *is*, not what the string looks like. These are exactly the sites a generic PHP language server can't see through `__get`. Call-form magic (`->active()` scopes, `whereEmail()` dynamic finders) isn't indexed yet. For the same counts anchored on the *declaration* side (the `posts()` method itself), see [Code Lens](code-lens.md) — each lens is click-to-open into this reference list.
