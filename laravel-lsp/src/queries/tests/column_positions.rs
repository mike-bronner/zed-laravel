...
fn view_column_positions() {
    ...
    assert_eq!(
        view.column, 12,
        "column should point to first char of view name"
    );
...
fn env_column_positions() {
    ...
    assert_eq!(
        env_call.end_column, 19,
        "end_column should be after last char"
    );
}
...
fn feature_column_positions() {
    ...
    if !patterns.feature_calls.is_empty() {
        ...
        assert_eq!(
            feature.column, 23,
            "column should point to first char of feature name"
        );
        assert_eq!(
            feature.end_column, 30,
            "end_column should be after last char"
        );
    }
}
...
fn blade_component_column_positions() {
    ...
    if !patterns.components.is_empty() {
        ...
        assert!(
            component.column < blade_code.len(),
...
fn double_quote_column_positions() {
    ...
    assert_eq!(
        view.column, 12,
        "column should point to first char inside quotes"
    );
    assert_eq!(view.end_column, 25, "end_column should be after last char");
}
...
fn calculate_string_column_range_directive_arg_shapes() {
    ...
    assert_eq!(
        result,
        Some((10, 14)),
        "@include('view') - 'view' at columns 10-14"
    );
    ...
    assert_eq!(
        result,
        Some((10, 14)),
        "Args without ( - parameter already past paren"
    );
    ...
    assert_eq!(
        result,
        Some((10, 14)),
        "Args without parens - parameter at quote"
    );
    ...
    assert_eq!(
        result,
        Some((14, 21)),
        "Indented directive at columns 14-21"
    );
